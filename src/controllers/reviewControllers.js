...
const getReviews = async (req, res) => {
    try {
            ...
            let reviewsDetails = await reviewModels.find({isDeleted:false})
            if (reviewsDetails.length == 0) return res.status(404).send({ status: false, message: "No reviews found" });
            return res.status(200).send({ status: true, message: "All Reviews list", "no. of reviews": reviewsDetails.length, data: reviewsDetails });
